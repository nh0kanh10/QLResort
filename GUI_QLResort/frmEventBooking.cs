...
namespace GUI_QLResort
{
    public partial class frmEventBooking : AppBaseForm
    {
        ...
        private GuestM selectedGuest = null;
        ...
        {
            lvGoiSuKien.Items.Clear();
            var result = eventPackageBUS.GetEventPackages(isActive: true);
        ...
        private void btnXacNhan_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Lấy mã chi tiết sự kiện vừa tạo để cập nhật đặt cọc và tạo hóa đơn
                ...
                if (eventDetails.Success && eventDetails.Data.Count > 0)
                {
                    ...
                    var invoiceResult = new InvoiceBUS().CreateInvoice(
                        ...
                        latestDetail.MaCTSK,     // maCTSK
                        "SuKien"                 // loaiHoaDon
                    );

                    if (invoiceResult.Success)
                    {
                        string maHD = invoiceResult.Data.MaHD;

                        // Nếu có đặt cọc, tạo thanh toán ngay
                        if (datCoc > 0)
                        {
                            var paymentResult = paymentBUS.AddPayment(
                                maHD,
                                datCoc,
                                (cbPhuongThucThanhToan.SelectedItem as dynamic)?.Key, // Lấy mã loại thanh toán
                                DateTime.Now
                            );

                            if (!paymentResult.Success)
                            {
                                MessageBox.Show($"Đặt sự kiện thành công nhưng lỗi tạo thanh toán đặt cọc: {paymentResult.ErrorMessage}", "Cảnh báo",
                                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                    }
                    ...
                    {
                         MessageBox.Show($"Đặt sự kiện thành công nhưng lỗi tạo hóa đơn: {invoiceResult.ErrorMessage}", "Cảnh báo",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
                else
                {
                    MessageBox.Show("Không thể lấy thông tin chi tiết sự kiện vừa tạo. Hóa đơn không được tạo.", "Cảnh báo",
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
