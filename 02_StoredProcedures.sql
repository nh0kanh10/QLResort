...
-- ==============================================================================
-- EVENT PACKAGE PROCEDURES (Additional - with MaSK)
-- ==============================================================================

-- Insert Event Package (with MaSK instead of LoaiSuKien)
...
BEGIN
    SET NOCOUNT ON;
    -- Get LoaiSuKien from SuKien table
    DECLARE @LoaiSuKien NVARCHAR(100);
    SELECT @LoaiSuKien = LoaiSuKien FROM SuKien WHERE MaSK = @MaSK;
    
    INSERT INTO GoiSuKien (MaGoiSK, TenGoiSK, LoaiSuKien, MoTa, GiaCoBan, SoKhachToiThieu, 
                          SoKhachToiDa, ThoiGianToiThieu, ThoiGianToiDa, DichVuKemTheo, 
                          IsGoiMacDinh, MaCN, CreatedBy, IsActive, CreatedAt)
    VALUES (@MaGoiSK, @TenGoiSK, @LoaiSuKien, @MoTa, @GiaGoi, @SoKhachToiThieu, 
            @SoKhachToiDa, @ThoiGianToiThieu, @ThoiGianToiDa, @DichVuKemTheo, 
            0, @MaCN, @CreatedBy, @IsActive, GETDATE());
END
GO

-- Update Event Package (with MaSK support)
CREATE OR ALTER PROC sp_UpdateEventPackage
    @MaGoiSK NVARCHAR(20),
    @MaSK NVARCHAR(20) = NULL,
    @TenGoiSK NVARCHAR(200) = NULL,
    @MoTa NVARCHAR(500) = NULL,
    @GiaGoi DECIMAL(12,2) = NULL,
    @SoKhachToiThieu INT = NULL,
    @SoKhachToiDa INT = NULL,
    @ThoiGianToiThieu INT = NULL,
    @ThoiGianToiDa INT = NULL,
...
BEGIN
    ...
    DECLARE @LoaiSuKien NVARCHAR(100) = NULL;
    IF @MaSK IS NOT NULL
    BEGIN
        SELECT @LoaiSuKien = LoaiSuKien FROM SuKien WHERE MaSK = @MaSK;
    END
    ...
    SET TenGoiSK = ISNULL(@TenGoiSK, TenGoiSK),
        LoaiSuKien = ISNULL(@LoaiSuKien, LoaiSuKien),
        MoTa = ISNULL(@MoTa, MoTa),
        GiaCoBan = ISNULL(@GiaGoi, GiaCoBan),
        SoKhachToiThieu = ISNULL(@SoKhachToiThieu, SoKhachToiThieu),
        SoKhachToiDa = ISNULL(@SoKhachToiDa, SoKhachToiDa),
        ThoiGianToiThieu = ISNULL(@ThoiGianToiThieu, ThoiGianToiThieu),
        ThoiGianToiDa = ISNULL(@ThoiGianToiDa, ThoiGianToiDa),
...
-- ==============================================================================
-- EVENT PROCEDURES (Additional - Simplified versions)
-- ==============================================================================

-- Get All Events (Event Types/Categories) - Alias for sp_GetSuKien
CREATE OR ALTER PROC sp_GetAllEvents
    @IsActive BIT = NULL
AS
BEGIN
    SET NOCOUNT ON;
    SELECT MaSK, TenSK, LoaiSuKien, MaCN, DiaDiem, GhiChu, TongChiPhi,
           CreatedAt, CreatedBy, UpdatedAt, UpdatedBy, IsActive
    FROM SuKien
    WHERE (@IsActive IS NULL OR IsActive = @IsActive)
    ORDER BY TenSK;
...
-- Get Events with filters - Alias for sp_GetSuKien
CREATE OR ALTER PROC sp_GetEvents
    @MaSK NVARCHAR(20) = NULL,
    @MaCN NVARCHAR(20) = NULL,
    @LoaiSuKien NVARCHAR(100) = NULL,
    @IsActive BIT = NULL
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetSuKien @MaSK, @MaCN, @LoaiSuKien, @IsActive;
END
GO
...
-- Insert Event (Simplified - only TenSK and GhiChu)
...
BEGIN
    SET NOCOUNT ON;
    INSERT INTO SuKien (MaSK, TenSK, LoaiSuKien, MaCN, GhiChu, CreatedBy, CreatedAt, IsActive)
    VALUES (@MaSK, @TenSK, N'Khác', 'CN01', @GhiChu, @CreatedBy, GETDATE(), 1);
END
GO

-- Update Event (Simplified)
...
-- ==============================================================================
-- EVENT PACKAGE PROCEDURES (Additional)
-- ==============================================================================
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetGoiSuKien NULL, NULL, NULL, NULL, @IsActive;
END
GO

-- Get Event Packages with filters
CREATE OR ALTER PROC sp_GetEventPackages
    @MaGoiSK NVARCHAR(20) = NULL,
    @LoaiSuKien NVARCHAR(100) = NULL,
    @MaCN NVARCHAR(20) = NULL,
    @IsGoiMacDinh BIT = NULL,
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetGoiSuKien @MaGoiSK, @LoaiSuKien, @MaCN, @IsGoiMacDinh, @IsActive;
END
GO

-- Get Event Packages By Event Type
...
BEGIN
    ...
    FROM GoiSuKien GSK
    LEFT JOIN ChiNhanh CN ON GSK.MaCN = CN.MaCN
    WHERE GSK.LoaiSuKien = (SELECT LoaiSuKien FROM SuKien WHERE MaSK = @MaSK)
      AND (@IsActive IS NULL OR GSK.IsActive = @IsActive)
    ORDER BY GSK.TenGoiSK;
END
GO

-- Get Event Package By ID
CREATE OR ALTER PROC sp_GetEventPackageByID
    @MaGoiSK NVARCHAR(20)
AS
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetGoiSuKien @MaGoiSK, NULL, NULL, NULL, NULL;
END
GO

-- ==============================================================================
-- EVENT DETAIL PROCEDURES (Additional)
-- ==============================================================================
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetCTSuKien NULL, NULL, NULL, NULL, @IsActive;
END
GO

-- Get Event Details with filters
CREATE OR ALTER PROC sp_GetEventDetails
    @MaCTSK NVARCHAR(20) = NULL,
    @MaSK NVARCHAR(20) = NULL,
    @MaKH NVARCHAR(20) = NULL,
    @TrangThai NVARCHAR(50) = NULL,
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetCTSuKien @MaCTSK, @MaSK, @MaKH, @TrangThai, @IsActive;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetCTSuKien @MaCTSK, NULL, NULL, NULL, NULL;
END
GO
...
BEGIN
    SET NOCOUNT ON;
    EXEC sp_GetCTSuKien NULL, NULL, @MaKH, NULL, @IsActive;
END
GO
...
-- ==========================================
-- STORED PROCEDURES CHO CHI TIẾT SỰ KIỆN (EVENT DETAIL) - ALIAS/TƯƠNG THÍCH
-- ==========================================

-- [PROC - EVENT DETAIL] Thêm mới (alias cho sp_InsertCTSuKien với parameters mapping)
-- Lưu ý: Đây là alias để tương thích với code cũ sử dụng EventDetail model
CREATE OR ALTER PROC sp_InsertEventDetail
    @MaCTSK NVARCHAR(20),
    @MaGoiSK NVARCHAR(20) = NULL,  -- Backward compatibility, sẽ map sang MaSK
    @MaKH NVARCHAR(20),
    @NgayToChuc DATETIME2 = NULL,
    @SoLuongKhach INT = 1,
    @YeuCauThem NVARCHAR(500) = NULL,
    @ChiPhiGoi DECIMAL(12,2) = 0,
    @ChiPhiPhatSinh DECIMAL(12,2) = 0,
    @DaThanhToan DECIMAL(12,2) = 0,
    @TrangThai NVARCHAR(50) = N'Lên kế hoạch',
    @GhiChu NVARCHAR(300) = NULL,
    @CreatedBy NVARCHAR(20) = NULL,
    @MaSK NVARCHAR(20) = NULL,  -- Dùng MaSK nếu có, nếu không thì lấy từ MaGoiSK
    @MaCTDV NVARCHAR(20) = NULL,
    @NgayBD DATETIME2 = NULL,
    @NgayKT DATETIME2 = NULL,
    @TongKhach INT = NULL
...
BEGIN
    ...
    -- Map parameters từ model cũ sang model mới
    DECLARE @MaSKFinal NVARCHAR(20) = ISNULL(@MaSK, @MaGoiSK);
    DECLARE @NgayBDFinal DATETIME2 = ISNULL(@NgayBD, @NgayToChuc);
    DECLARE @NgayKTFinal DATETIME2 = ISNULL(@NgayKT, DATEADD(DAY, 1, @NgayToChuc));
    DECLARE @DonGiaFinal DECIMAL(12,2) = @ChiPhiGoi;
    DECLARE @SoLuongFinal INT = @SoLuongKhach;
    DECLARE @TongKhachFinal INT = ISNULL(@TongKhach, @SoLuongKhach);
    
    -- Gọi stored procedure thực tế
    EXEC sp_InsertCTSuKien
        @MaCTSK = @MaCTSK,
        @MaSK = @MaSKFinal,
        @MaKH = @MaKH,
        @MaCTDV = @MaCTDV,
        @SoLuong = @SoLuongFinal,
        @DonGia = @DonGiaFinal,
        @GhiChu = @GhiChu,
        ...
        @NgayBD = @NgayBDFinal,
        @NgayKT = @NgayKTFinal,
        @TongKhach = @TongKhachFinal,
        @CreatedBy = @CreatedBy,
        @IsActive = 1;
END
GO

-- [PROC - EVENT DETAIL] Cập nhật (alias cho sp_UpdateCTSuKien)
CREATE OR ALTER PROC sp_UpdateEventDetail
    @MaCTSK NVARCHAR(20),
    @NgayToChuc DATETIME2 = NULL,
    @SoLuongKhach INT = NULL,
    @YeuCauThem NVARCHAR(500) = NULL,
    @ChiPhiPhatSinh DECIMAL(12,2) = NULL,
    @DaThanhToan DECIMAL(12,2) = NULL,
    @TrangThai NVARCHAR(50) = NULL,
    @GhiChu NVARCHAR(300) = NULL,
    @UpdatedBy NVARCHAR(20) = NULL,
...
-- [PROC - EVENT DETAIL] Cập nhật thanh toán
...
BEGIN
    SET NOCOUNT ON;
    UPDATE CTSuKien
    SET DaThanhToan = ISNULL(DaThanhToan, 0) + @SoTienTra,
        UpdatedBy = @UpdatedBy,
        UpdatedAt = GETDATE()
    WHERE MaCTSK = @MaCTSK;
END
...
-- [PROC - EVENT DETAIL] Xóa (soft delete)
...
-- [PROC - EVENT DETAIL] Lấy theo khoảng thời gian
...
BEGIN
    ...
    WHERE CTSK.IsActive = 1
      AND (@TrangThai IS NULL OR CTSK.TrangThai = @TrangThai)
      AND (CTSK.NgayBD >= @NgayBatDau AND CTSK.NgayBD <= @NgayKetThuc
           OR CTSK.NgayKT >= @NgayBatDau AND CTSK.NgayKT <= @NgayKetThuc
           OR CTSK.NgayBD <= @NgayBatDau AND CTSK.NgayKT >= @NgayKetThuc)
    ORDER BY CTSK.NgayBD DESC;
END
GO