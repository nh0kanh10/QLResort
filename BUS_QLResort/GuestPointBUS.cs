...
using DAL_QLResort.GuestPointDAL;
using System;
using System.Data;
...
namespace BUS_QLResort
{
    public class GuestPointBUS
    {
        ...
        public OperationResult<bool> AddPoints(string maKH, int diemCong, string ghiChu)
        {
            if (string.IsNullOrWhiteSpace(maKH))
                return OperationResult<bool>.Fail("Mã khách hàng không được để trống");

            if (diemCong <= 0)
                return OperationResult<bool>.Fail("Số điểm cộng phải lớn hơn 0");

            var result = guestPointDAL.AddPoints(maKH, diemCong, ghiChu, Session_Now.CurrentUser, Session_Now.CurrentUser);
            return result;
        }
    }
}
