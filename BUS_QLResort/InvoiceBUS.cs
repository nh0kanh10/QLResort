...
using DAL_QLResort.PromotionDAL;
using System;
...
namespace BUS_QLResort
{
    public class InvoiceBUS
    {
        ...
        public OperationResult<Invoice> CreateInvoice(string maDP, string maKH, string maNV, string maCN, 
            decimal tongTruocKM, string couponCode = null, string maLKH = null, string maLP = null, string maPhong = null, string maCTSK = null, string loaiHoaDon = "DatPhong")
        {
            if (string.IsNullOrWhiteSpace(maDP) && string.IsNullOrWhiteSpace(maCTSK))
                return OperationResult<Invoice>.Fail("Phải có Mã đặt phòng hoặc Mã sự kiện");
        ...
        {
            var invoices = GetInvoices(); // Lấy tất cả hóa đơn
            int maxNumber = 0;
            ...
            string newCode = $"HD{nextNumber:D3}";

            // Loop verify to ensure uniqueness
            while (existingCodes.Contains(newCode))
            {
                nextNumber++;
                newCode = $"HD{nextNumber:D3}";
            }
